...
(function( root, factory ) {
	...
	var SLIDES_SELECTOR = '.slides section',
		...
		config = {
			...
			// Display the page number of the current slide // Switched on by Peter Petrov
			slideNumber: true,
			...
			mouseWheel: ismac,									// Changed on by Peter Petrov

			// Apply a 3D roll to links on hover
			rollingLinks: false, 								// Switched on by Peter Petrov
			...
			viewDistance: 2, // Changed by Peter Petrov
		...
		keyboardShortcuts = {
			'Esc':			'Sitemap / Карта на сайта', 		//
			'Home':			'Home page / Начална страница',		//
			'C':			'Contact / Контакт', 				//
			'F':			'Fullscreen / Цял екран',  			//
			'&#8592;':		'Navigate left / Наляво', 			//
			'&#8594;':		'Navigate right / Надясно', 		//
			'&#8593;':		'Navigate up / Нагоре', 			//
			'&#8595;':		'Navigate down / Надолу',   		// Changed by Peter Petrov
			'?':			'This Navigation Help / Този екран' //
		};
...
//  		window.addEventListener('orientationchange', doOnOrientationChange);
	...
	function addEventListeners() {
		...
		window.addEventListener( 'orientationchange', onOrientationChange, false ); // Added by Peter Petrov
		...
		pointerEvents.forEach( function( eventName ) {
			...
			dom.controlsMenu.forEach( function( el ) { el.addEventListener( eventName, onMenuClicked, false ); } ); // Added by Peter Petrov
		} );
	...
	function removeEventListeners() {
		...
		window.removeEventListener( 'orientationchange', onOrientationChange, false ); // Added by Peter Petrov
		...
		[ 'touchstart', 'click' ].forEach( function( eventName ) {
			...
			dom.controlsMenu.forEach( function( el ) { el.removeEventListener( eventName, onMenuClicked, false ); } ); // Added by Peter Petrov
		} );
	...
	function hideAddressBar() {
		...
		if( config.hideAddressBar && isMobileDevice ) {
			...
			window.addEventListener( 'orientationchange', removeAddressBar, false ); // Added by Peter Petrov
		}
	...
	function showHelp() {
		...
		if( config.help ) {
			...
			var html = '<p class="title">Keyboard Shortcuts<br>Клавиши за бърз достъп</p><br/>';

			html += '<table>'; // Changed by Peter Petrov
			for( var key in keyboardShortcuts ) {
				html += '<tr><td class="c1"><center><div class="c1d">' + key + '</center></div></td><td class="c2"><div class="c2d">' + keyboardShortcuts[ key ] + '</div></td></tr>'; // Changed by Peter Petrov
			}
	...
	function updateSlideNumber() {
		...
		var sections = document.querySelectorAll(".reveal .slides #prosection section").length; // Added by Peter Petrov
		var language = 1;
		...
		if(document.querySelectorAll(".reveal .slides section")[sections-1].className == "present"){ // Added by Peter Petrov
			document.getElementById('mapsec').innerHTML+='<iframe id="bgmap" class="bg-map" style="position: absolute; top: 101px; left: 342px; z-index: '+zi+';" src="https://www.google.com/maps/d/embed?mid=zFDFuEjbw6J0.ks3wVDYjA7Uw" width="560" height="420"></iframe><iframe id="enmap" class="en-map" style="position: absolute; top: 101px; left: 342px; z-index: 1;" src="https://www.google.com/maps/d/embed?mid=zFDFuEjbw6J0.ke1TNTelLCJA" width="560" height="420"></iframe>'}; // Added by Peter Petrov
	...
	function getSlidePastCount() {
		...
		var pastCount = 1; // Changed by Peter Petrov from 0
	...
	function getProgress() {
		...
		return pastCount / totalCount; // Changed by Peter Petrov
	...
	function onDocumentKeyPress( event ) {
		...
		if( event.shiftKey && event.charCode === 63 || event.keyCode === 63 ) {
			if( dom.overlay ) {
//				closeOverlay(); // Changed by Peter Petrov
			}
			else {
//				showHelp( true ); // Changed by Peter Petrov
			}
		}
...
case 40: navigateDown(); break; //
				...
				// c
//				case 67: slide( Number.MAX_VALUE ); break;
				// space
...
//				case 65: if ( config.autoSlideStoppable ) toggleAutoSlide( autoSlideWasPaused ); break;
		...
		// ESC 
		else if ( ( event.keyCode === 27 ) && features.transforms3d ) { // Changed by Peter Petrov
			if( dom.overlay ) {
				closeOverlay();
			}
			else {
				if(!isMobileDevice){toggleOverview();};
			}
...
//		slide( slideIndex ); // Changed by Peter Petrov
	...
	function onOrientationChange( event ) {							// 
	    switch(window.orientation){  								// 
	      case -90:													// 
	      case 90:													// 
	      	for(var i=0; i<=document.querySelectorAll(".reveal a.navigate-down.arrow.enabled").length - 1; i++){// 
	        	document.querySelectorAll(".reveal a.navigate-down.arrow.enabled")[i].style.display = "none";	// 
	   		 }																													// 
	      ...
	      default:																												// 
	      	for(var i=0; i<=document.querySelectorAll(".reveal a.navigate-down.arrow.enabled").length - 1; i++){// 
	        	document.querySelectorAll(".reveal a.navigate-down.arrow.enabled")[i].style.display = "inline-block";// 
	   		 }																													// 
	      	for(var i=0; i<=document.querySelectorAll(".reveal .quest").length - 1; i++){										// 
