	...
	create: function( options ) {
		return (function(){
			...
			function setupPositions() {
				...
				contentsAngle = (config.angle + plus) / -2; // Changed by Peter Petrov
				switch( config.position ) {
					...
					case POSITION_R:
						...
						menuTransformClosed = 'rotateY( ' + menuAngle + 'deg ) translateX( 100% ) translateX( 0px ) scale( 1.0 )';
			...
			function close() {
				if( isOpen ) {
					...
					document.querySelectorAll(".meny-active .meny-arrow")[0].style.right = "0px"; // Added by Peter Petrov
					...
					if( supports3DTransforms ) {
						...
						if(c.substring(0,5)=="gecko"){
							document.getElementsByClassName("meny")[0].style.transform=menuTransformClosed;
							document.getElementsByClassName("contents")[0].style.transform=contentsTransformClosed;
							}
						else{
				...
				if( !isMouseDown ) {
					...
					switch( config.position ) {
						case POSITION_T:
							...
							else if( y < config.threshold && zoomflag === 0 ) { // Changed by Peter Petrov && zoomflag === 0
								open();
							}
							break;
						...
						case POSITION_R:
							...
							else if( x > w - config.threshold && zoomflag === 0 && y < dom.wrapper.offsetHeight/2 + 120 && y > dom.wrapper.offsetHeight/2 - 120) { // Changed by Peter Petrov && zoomflag === 0 && y < dom.wrapper.offsetHeight - 110
							open();
							}
							break;
						...
						case POSITION_B:
							...
							else if( y > h - config.threshold && zoomflag === 0 ) { // Changed by Peter Petrov && zoomflag === 0
								open();
							}
							break;
						...
						case POSITION_L:
							...
							else if( x < config.threshold && zoomflag === 0 ) { // Changed by Peter Petrov && zoomflag === 0
								open();
							}
							break;
					}
